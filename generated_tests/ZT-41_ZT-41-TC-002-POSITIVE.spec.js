...
// Test ID: ZT-41-TC-002-POSITIVE
// Type: positive, Priority: P1

const baseUrl = process.env.APP_BASE_URL || 'http://localhost:3000';

test.describe('ZT-41: User successfully registers with another valid credentials', () => {
  ...
  test('User successfully registers with another valid credentials', async ({ page }) => {
    ...
    await page.locator('input[type="text"], textarea').first().fill('Jane Doe');
    // Step 3: Enter valid email address in the email field
    // Expected: Email field is populated with the entered email
    await page.getByRole('textbox', { name: /email|username/i }).or(page.locator('#username, #email, input[type=email]')).first().fill('janedoe@example.com');
