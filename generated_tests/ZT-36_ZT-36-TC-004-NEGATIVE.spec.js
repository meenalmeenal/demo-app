...
// Test ID: ZT-36-TC-004-NEGATIVE
// Type: negative, Priority: P2

const baseUrl = process.env.APP_BASE_URL || 'http://localhost:3000';

test.describe('ZT-36: User registration fails with invalid email address', () => {
  ...
  test('User registration fails with invalid email address', async ({ page }) => {
    ...
    // Expected: Confirm password field shows masked characters, register button is disabled
    await page.getByRole('textbox', { name: /password/i }).or(page.locator('#password, input[type=password]')).first().fill('SecurePass123!');
    // Step 6: Click the register button
    // Expected: Error message displays indicating that email address is invalid
