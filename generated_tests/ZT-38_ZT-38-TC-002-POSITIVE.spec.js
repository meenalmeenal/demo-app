...
// Auto-generated test case for ZT-38
// Test ID: ZT-38-TC-002-POSITIVE
// Type: positive, Priority: P1

const baseUrl = process.env.APP_BASE_URL || 'http://localhost:3000';

test.describe('ZT-38: User successfully registers with valid credentials and speci', () => {
  ...
  test('User successfully registers with valid credentials and special characters', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: /email|username/i }).or(page.locator('#username, #email, input[type=email]')).first().fill('johndoe@example.co.uk');
    // Step 4: Enter valid password in the password field
    // Expected: Password field shows masked characters
    await page.getByRole('textbox', { name: /password/i }).or(page.locator('#password, input[type=password]')).first().fill('SecureP@ss123!');
