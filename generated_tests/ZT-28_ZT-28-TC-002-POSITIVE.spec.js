...
// Test ID: ZT-28-TC-002-POSITIVE
// Type: positive, Priority: P1

const baseUrl = process.env.APP_BASE_URL || 'http://localhost:3000';

test.describe('ZT-28: User logs in successfully without remember me option', () => {
  ...
  test('User logs in successfully without remember me option', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: /email|username/i }).or(page.locator('#username, #email, input[type=email]')).first().fill('testuser2@example.com');
    ...
    await page.getByRole('textbox', { name: /password/i }).or(page.locator('#password, input[type=password]')).first().fill('SecurePass123!2');
    ...
    // Step 6: Verify user session is established without remember me option
