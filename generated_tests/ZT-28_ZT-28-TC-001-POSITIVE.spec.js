...
// Test ID: ZT-28-TC-001-POSITIVE
// Type: positive, Priority: P1

const baseUrl = process.env.APP_BASE_URL || 'http://localhost:3000';

test.describe('ZT-28: User successfully logs in with valid credentials and remembe', () => {
  ...
  test('User successfully logs in with valid credentials and remember me option', async ({ page }) => {
    ...
    // Step 6: Verify user session is established and remember me option is applied
    // Expected: User profile icon is visible in header, logout option is available, and user remains logged in after closing and reopening the browser
    await expect(page.locator('body')).toBeVisible();
  });
});