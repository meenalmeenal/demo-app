...
// Test ID: ZT-29-TC-005-BOUNDARY
// Type: boundary, Priority: P3

const baseUrl = process.env.APP_BASE_URL || 'http://localhost:3000';

test.describe('ZT-29: Login with maximum length email address', () => {
  ...
  test('Login with maximum length email address', async ({ page }) => {
    ...
    await page.goto(baseUrl);
    // Step 2: Enter a long email address in the email field
    // Expected: Email field is populated with the entered long email address
    await page.getByRole('textbox', { name: /email|username/i }).or(page.locator('#username, #email, input[type=email]')).first().fill('verylongemailaddresswithmaxallowedlength@example.com');
